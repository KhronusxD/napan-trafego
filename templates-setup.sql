-- Step 1: Adicionar coluna template_id à tabela companies (se não existir)
ALTER TABLE companies ADD COLUMN IF NOT EXISTS template_id TEXT;

-- Step 2: Inserir os 3 templates padrão
...
-- Step 3: Atualizar empresas existentes para usar os templates
UPDATE companies SET template_id = 'default' WHERE type = 'default' AND template_id IS NULL;
UPDATE companies SET template_id = 'whatsapp' WHERE type = 'whatsapp' AND template_id IS NULL;
UPDATE companies SET template_id = 'servicos' WHERE type = 'itv-manaus' AND template_id IS NULL;
